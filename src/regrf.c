...
void regRF(double *x, double *y, int *xdim, int *sampsize,
	   ...
	   int *cat, int *maxcat, int *jprint, int *doProx, int *oobprox,
           ...
           int *nout, int *inbag, double *ssnode, double *decSplit,
           int *maxLevel, int *groupOut, double *permX, int *corr) {
    /*************************************************************************
   Input:
  ...
  *************************************************************************/
    ...
    int k, m, mr, n, nOOB, j, jout, idx, ntest, last, ktmp, nPerm,
        nsample, mdim, keepF, keepInbag, ngroup, keepGroup;
    int *oobpair, varImp, localImp, *varUsed;

    int *in, *nind, *nodex, *nodexts, *group;
    ...
    group      = (int *) S_alloc(nsample, sizeof(int));
    ...
    for (j = 0; j < *nTree; ++j) {
		...
		regTree(xb, yb, mdim, *sampsize, lDaughter + idx, rDaughter + idx,
                ...
                varUsed, ssnode + idx, decSplit + idx);
		...
		if (varImp) {
			for (mr = 0; mr < mdim; ++mr) {
                if (varUsed[mr]) { /* Go ahead if the variable is used */
                    ...
                    ooberrperm = 0.0;
                    if (*maxLevel > 0) {
                      ngroup = groupRegTreeCross(x, nsample, mdim,
                         nodestatus + idx, upper + idx, mbest + idx,
                         treeSize[j], cat, *maxcat, mr, group, *maxLevel,
                         corr);
                  		if (keepGroup && j == *nTree-1) {
                  			for (n = 0; n < nsample; ++n)
                          groupOut[n + mr * nsample] = group[n];
                  		}
                    }
                    for (k = 0; k < nPerm; ++k) {
                        if (*maxLevel == 0) {
                            ...
                            permuteOOBGroup(mr, x, in, nsample, mdim,
                                            group, ngroup);
                        }
                        predictRegTree(x, nsample, mdim, lDaughter + idx,
                    ...
                    /* retain permuted data for last tree */
                		if (keepGroup && j == *nTree-1) {
                			for (n = 0; n < nsample; ++n)
                        permX[n + mr * nsample] = x[mr + n * mdim];
                		}
