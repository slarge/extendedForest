...
void classRF(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
	...
	keepInbag, nstrata, keepGroup, ngroup;
	...
	*nrightimp, *nout, *nclts, Ntree;
    ...
    int *out, *bestsplitnext, *bestsplit, *nodepop, *jin, *nodex, *group,
	...
	*at, *a, *b, *mind, *nind, *jts, *oobpair;
	...
	do {
	    ...
	    zeroDouble(decsplit + idxByNnode, *nrnodes);
	...
	if (imp) {
        ...
        if (*maxLevel > 0) {
          ngroup = groupRegTreeCross(x, nsample, mdim,
             nodestatus + idxByNnode, xbestsplit + idxByNnode,
             bestvar + idxByNnode,ndbigtree[jb], cat, *maxcat, m, group,
             *maxLevel, corr);
      		if (keepGroup && jb == Ntree-1) {
      			for (n = 0; n < nsample; ++n)
              groupOut[n + m * nsample] = group[n];
      		}
        }
        ...
        if (*maxLevel == 0) {
          ...
          permuteOOBGroup(m, x, jin, nsample, mdim, group, ngroup);
        }
    		if (keepGroup && jb == Ntree-1) {
    			for (n = 0; n < nsample; ++n)
            permX[n + m * nsample] = x[m + n * mdim];
    		}
